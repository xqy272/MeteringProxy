...
生产首发使用单实例 SQLite 写入模型。同一个 `/data/usage.sqlite` 不应被两个 MeteringProxy 实例同时写入；升级和回滚都采用先停止旧实例、再启动新实例的方式。

### 1. 前提条件

- 已安装 Docker 的 Linux 服务器
- Caddy（或其他反向代理）已部署在服务器上
- CLIProxyAPI 已在 Docker 中运行

### 2. 创建共享网络

两个容器需要互通——MeteringProxy 通过容器名访问 CLIProxyAPI：
...
> 将 `<cli-proxy-container-name>` 替换为 CLIProxyAPI 容器的实际名称（通过 `docker ps --format '{{.Names}}'` 查看）。`config.yaml` 中的 `upstream` 也要与此名称一致，例如容器名是 `cli-proxy-api`，则 `upstream: "http://cli-proxy-api:8317"`。
>
> 容器间通信走的是 Docker 内部网络，不受 `-p` 绑定地址（如 `127.0.0.1:8317:8317`）的影响。即使 CLIProxyAPI 只绑定到宿主机 localhost，同一网络内的其他容器依然能通过容器名 + 内部端口直接访问。
>
> 如果 CLIProxyAPI 不在 Docker 中（裸机部署），参考[附录 A：裸机 CLIProxyAPI](#附录-a裸机-cliproxyapi)。

### 3. 准备主机目录

```bash
mkdir -p /opt/ai-gateway/metering/backups
chown -R 1000:1000 /opt/ai-gateway/metering   # 与容器内 appuser 的 uid:gid 对应
...
请保持 `pricing.yaml` 与服务商合同定价同步。价格表只在进程启动时加载；修改 `pricing.yaml` 后必须重启服务，WebUI 的成本估算才会刷新。
...
如果迁移导致数据库不可用，必须先停止服务，再恢复上线前备份。WAL 模式下不能只覆盖主库文件而保留旧的 sidecar 文件：
...
`--add-host host.docker.internal:host-gateway` 在容器 `/etc/hosts` 中添加一条记录，将 `host.docker.internal` 解析到宿主机 IP，使容器内的 `upstream` 能通过宿主机的 8317 端口访问 CLIProxyAPI。
...
- [ ] 备份已手动执行一次，且备份文件通过 `PRAGMA integrity_check`
- [ ] 数据库恢复演练确认会先删除旧 `usage.sqlite-wal` 和 `usage.sqlite-shm`
- [ ] 回滚所需的上一镜像 tag、SQLite 备份路径和 Caddy 配置变更已准备就绪
