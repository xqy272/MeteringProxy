...
WebUI 为只读面板，不修改配置或数据库。页面使用 Tabs 模式，默认展示请求总览、成本/Token/请求趋势、模型分布、图片计量、API Key 维度、近期问题、凭证健康、额度/凭证刷新诊断与采集诊断。最近 100 条请求明细默认隐藏，仅在点击展开或从 issue 卡片进入时查询。WebUI 支持中英文切换，语言偏好保存在浏览器本地；页面右上角提供项目 GitHub 链接。
