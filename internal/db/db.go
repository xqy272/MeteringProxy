...
func (db *DB) RecentQuotaRefreshEvents(ctx context.Context, since time.Time, limit int) ([]QuotaRefreshEventRow, error) {
	return db.RecentQuotaRefreshEventsReport(ctx, since, limit)
}
...
func (db *DB) SideUsageStatusCounts(ctx context.Context, since time.Time) (map[string]int64, error) {
	return db.SideUsageStatusCountsReport(ctx, since)
}
